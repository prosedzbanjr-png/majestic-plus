  ...
  return (
    <main className={styles.page}>
      ...
      <section className={styles.hero}>
        <div className={styles.heroArtwork} style={{ backgroundImage: `url("${heroImage}")` }} /><div className={styles.heroShade} /><div className={styles.heroGrain} />
        <button className={`${styles.arrow} ${styles.arrowLeft}`} aria-label="Poprzednia produkcja">‹</button><button className={`${styles.arrow} ${styles.arrowRight}`} aria-label="Następna produkcja">›</button>
        <div className={styles.heroContent}>
          <div className={styles.original}>{featured.original ? "MAJESTIC+ ORIGINAL" : "RICHARDS MAJESTIC"}</div>
          <h1 className={styles.heroTitle}>{featured.title.split(" ").map((part, index) => <span key={`${part}-${index}`}>{part}</span>)}</h1>
          <div className={styles.meta}><span className={styles.match}>{matchLabel}</span><span>{featured.year}</span><span className={styles.maturity}>{featured.maturity}</span><span>{featured.runtime}</span><span>{featured.quality}</span>{featured.contentType === "series" && <span>SERIAL</span>}</div>
          <p className={styles.description}>{featured.description}</p>
          <div className={styles.heroButtons}><Link className={styles.primary} href={primaryHref}><span>▶</span>{featured.contentType === "series" ? "Zobacz serial" : "Oglądaj"}</Link><Link className={styles.secondary} href={`/title/${featured.slug}`}><span>＋</span>Więcej informacji</Link></div>
        </div>
        <div className={styles.pager} aria-hidden="true"><span className={styles.pagerActive} /><span /><span /><span /></div>
      </section>
